...
export default async function SchemaDetailPage({ params }: PageProps) {
  ...
  const allSchemaNames = getAllSchemaNames();
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex gap-4 flex-wrap">
            {schema.extends && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200">
                {t('extendsLabel', { parent: schema.extends })}
              </span>
            )}
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
              {t('totalProperties', { count: schema.properties.length + (schema.inheritedProperties?.length || 0) })}
            </span>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden mb-8">
          <div className="px-8 py-6 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('properties')}</h2>
            {schema.extends && (
              <p className="text-sm text-muted-foreground mt-2">
                {t('includesInherited', { parent: schema.extends })}
              </p>
            )}
          </div>
          <PropertyTable properties={schema.properties} allSchemaNames={allSchemaNames} />
        </div>
        ...
        {schema.examples && schema.examples.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('examples')}</h2>
            </div>
            <div className="p-8">
